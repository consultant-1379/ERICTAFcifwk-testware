...
public class GenericUIOperator {
    ...
    public BrowserTab startBrowser(String hostName, String location, String packageName){
        ...
        String port = portMap.get(Ports.HTTPS);
        if (port == null) {
            throw new IllegalArgumentException("HTTPS port not defined for host 'ciportal'");
        }
        String url = String.format("https://%s:%s", host.getIp(), port);
